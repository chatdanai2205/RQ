const ctx8_1 = document.getElementById('pie_chart8_1').getContext('2d');

const cfg8_1 = {
    type: 'bar',
    data: {
      labels: chronic_disease_1.map(item => item.chronic_disease), // ชื่อเพศ
      datasets: [{
          label: 'คะแนนเฉลี่ยรวม',
          data: chronic_disease_1.map(item => item.avg_total_score), // ดึงข้อมูล
          backgroundColor: '#8dc4ff',
          datalabels: {
            color: function(context) {
              const value = context.dataset.data[context.dataIndex]; // ค่าของตัวเลขในแต่ละแท่ง
              const category = context.dataset.label; // รับข้อมูล category เช่น "all", "pressure_tolerance" เป็นต้น
      ...
      scales: {
        x: {
            title: {
                ...
                text: 'โรคประจำตัว', // 🔹 กำหนดข้อความหมายเหตุที่แกน X
  ...
  // สร้างกราฟ
  const pie_chart8_1 = new Chart(ctx8_1, cfg8_1);
