const ctx5_1 = document.getElementById('pie_chart5_1').getContext('2d');
const label2 = gender_1.map(item => item.gender);

// กำหนดค่า configuration ของกราฟ
const cfg5_1 = {
  type: 'bar',
  data: {
    labels: gender_1.map(item => item.gender), // ชื่อเพศ
    datasets: [{
        label: 'คะแนนเฉลี่ยรวม',
        data: gender_1.map(item => item.avg_total_score), // เพิ่มค่าป้องกัน undefined
  ...
  options: {
    ...
    plugins: {
      ...
      legend: {
        display: true, // ✅ เปิด legend ให้แสดง
        labels: {
    ...
    scales: {
        x: {
            title: {
                ...
                text: 'เพศ', // 🔹 กำหนดข้อความหมายเหตุที่แกน X
...
// สร้างกราฟ
const pie_chart5_1 = new Chart(ctx5_1, cfg5_1);
