const ctx13_1 = document.getElementById('pie_chart13_1').getContext('2d');

// กำหนดค่า configuration ของกราฟ
const cfg13_1 = {
  type: 'bar',
  data: {
    labels: course_1.map(item => item.course), // ชื่อเพศ
    datasets: [{
        label: 'คะแนนเฉลี่ยรวม',
        data: course_1.map(item => item.avg_total_score || 0), // เพิ่มค่าป้องกัน undefined
    ...
    scales: {
        x: {
            title: {
              ...
              text: 'ความตอบโจทย์ของหลักสูตร', // 🔹 กำหนดข้อความหมายเหตุที่แกน X
...
// สร้างกราฟ
const pie_chart13_1 = new Chart(ctx13_1, cfg13_1);