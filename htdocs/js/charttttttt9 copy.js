const ctx9_1 = document.getElementById('pie_chart9_1').getContext('2d');

// กำหนดค่า configuration ของกราฟ
const cfg9_1 = {
  type: 'bar',
  data: {
    labels: fam_status_1.map(item => item.fam_status), // ชื่อเพศ
    datasets: [{
        label: 'คะแนนเฉลี่ยรวม',
        data: fam_status_1.map(item => item.avg_total_score || 0), // เพิ่มค่าป้องกัน undefined
    ...
    scales: {
        x: {
            title: {
                ...
                text: 'สถานภาพของครอบครัว', // 🔹 กำหนดข้อความหมายเหตุที่แกน X
...
// สร้างกราฟ
const pie_chart9_1 = new Chart(ctx9_1, cfg9_1);